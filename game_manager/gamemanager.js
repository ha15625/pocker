...
const percentIncrease = (partnumber, totalnumber) => Math.ceil((partnumber / totalnumber) * 100);
                        ...
                        collection.findOne(query, function (err, result) {
                            if (err) console.log("error1", err);
                                ...
                                if (result != null) {
                                    ...
                                    let buff = Friends.filter(x => x.accepted == true || x.accepted == "true");

                                    let arr1 = Friends.filter((o, index) => Friends.find((_o, _index) => o.id === _o.id && index !== _index))
                                    let arr2 = Friends.filter((o, index) => !Friends.find((_o, _index) => o.id === _o.id && index !== _index))
                                    if (arr1.length > 1)
                                        arr2.push(arr1[0]);
                                    setTimeout(() => {
...
exports.addroom = function (r_roomID, r_title, r_seatlimit, r_status, r_game_mode, r_buyin_min, r_buyin_max, SB, BB) {
    let inputplayerlist = [];
    let waitPlayerlist = [];
...
exports.playerenterroom = function (roomid, username, balance, avatar, photoURL, photoType, socket) {
        ...
        for (let index = 0; index < roomlist.length; index++) {
            if (roomlist[index].roomid == roomid) {
                ...
                if (roomlist[index].playerlist.length == -1) {
                    ...
                    while (true) {
                        if (roomlist[index].table.players[pos] == undefined || roomlist[index].table.players[pos].playerName == "Empty seat") {
    ...
    if (roomlist.length > 0) {
        for (let index = 0; index < roomlist.length; index++) {
            if (roomlist[index].roomid == roomid) {
                ...
                let seatedPlayers = roomlist[index].playerlist.filter(x => x.leaveenterflag == 1);
                let checknum = 0;
                if (roomlist[index].gamemode == 'tournament')
                    checknum = 4;
                else
                ...
                setTimeout(() => {
                    ...
                    getUsername(useridArray, function (data) {
                        ...
                        io.sockets.in('r' + roomlist[index].roomid).emit('CURRENT_ROOM_NAMES', { result: usernames });
                    });
                    io.sockets.in('r' + roomlist[index].roomid).emit('CURRENT_ROOM_STATUS', mydata);
                    let takeSeats = [];
                    for (let i = 0; i < roomlist[index].playerlist.length; i++) {
                        const element = roomlist[index].playerlist[i];
            ...
            if (roomlist[index].roomid == roomid) {
                for (let i = 0; i < roomlist[index].playerlist.length; i++) {
                    ...
                    if (element.username == username) {
                        ...
                        io.sockets.in('r' + roomlist[index].roomid).emit('REQ_TAKE_SEAT_RESULT', emitdata);
        ...
        if (roomlist[index].roomid == roomid) {
            for (let i = 0; i < roomlist[index].playerlist.length; i++) {
                ...
                if (element.username == username) {
                    ...
                    io.sockets.in('r' + roomlist[index].roomid).emit('REQ_CANCEL_TAKE_SEAT_RESULT', emitdata);
    ...
    for (let index = 0; index < roomlist.length; index++) {
        if (roomlist[index].roomid == roomid) {
            ...
            if (roomlist[index].table.started) {
                ...
                player.isSeated = !status;
                if (status == false) {
                    let seatedPlayers = roomlist[index].table.players.filter(x => !x.isEmptySeat);
...
function getCurrentRoomStatus(index, socket) {
    ...
    setTimeout(() => {
        ...
        if (socket != null)
            socket.emit('TAKE_SEAT_PLAYERS', emitdata);
    }, 100);
};
...
function roundstart(index) {
    if (roomlist[index].table.started == false) {
        ...
        console.log("room", index, ">>>", roomlist[index].playerlist.length, roomlist[index].table.players.length);
        ...
        roomlist[index].table.on("smallBlind", function (player) {
            for (let i = 0; i < roomlist[index].playerlist.length; i++) {
                ...
                if (element.username == player.playerName) {
                    ...
                    let emitdata;
                    let message = player.playerName + " posted small blind with $" + ChangeUnit(player.table.smallBlind)
                    ...
                    getUsername(useridArray, function (data) {
                        ...
                        io.sockets.in('r' + roomlist[index].roomid).emit('SmallBlind_Deal', emitdata);
                        io.sockets.in('r' + roomlist[index].roomid).emit('CURRENT_ROOM_NAMES', { result: usernames });
                    });
                }
            }
        ...
        roomlist[index].table.on("bigBlind", function (player) {
            ...
            let message = player.playerName + " posted big blind with $" + ChangeUnit(player.table.bigBlind)
            let emitdata = {
            ...
            io.sockets.in('r' + roomlist[index].roomid).emit('BigBlind_Deal', emitdata);
                ...
                if (element.username == player.playerName) {
                    ...
                    if (p) {
                        ...
                        if (pos != roomlist[index].table.sBPos && p.missBlind > 0) {
                            let num = p.missBlind;
                            roomlist[index].table.game.bets[pos] += roomlist[index].table.bigBlind;
                            ...
                            p.missBlind = 0;
                            element.waitforbb = 1;
                            let message1 = element.username + " called with $" + ChangeUnit(player.table.bigBlind) + "for waitforbb"
                            let emitdata = {
                                roomid: roomlist[index].roomid,
                            ...
                            io.sockets.in('r' + roomlist[index].roomid).emit('WAIT_FOR_BB', emitdata);
                        }
            ...
            setTimeout(() => {
                ...
                if (seatBots.length > 0 && seatnormals.length > 1) {
                    ...
                    for (i = 0; i < seatBots.length; i++) {
                        (function (x) {
                            let b = roomlist[index].table.getPlayerByName(seatBots[x].username);
                            if (b && b == player) {
                                setTimeout(function () {
        ...
        roomlist[index].table.on("dealCards", function (boardCardCount, currnt_bets) {
            ...
            roomlist[index].mainPots = exports.getMainPots(roomlist[index].mainPots, mainpots);
            let emitdata = null;
        ...
        roomlist[index].table.on("win", function (winner, prize) {
            setTimeout(() => {
                ...
                setTimeout(() => {
                    Record_Won_History(winner.playerName, prize, wining_hand, wining_cards, handrankVal);
                }, 100);
                console.log("gameWin::");
                ...
                setTimeout(() => {
                        ...
                        if (i < roomlist[index].playerlist.length) {
                            ...
                            if (element != undefined) {
                                if (element.buyinflag == 0) {
                                    let player = roomlist[index].table.getPlayerByName(element.username);
                                        ...
                                        if (element.getCorrectSeatnumber == 1) {
                                            if (element.waitforbb == 0) {
                                                ...
                                                if (!player) {
                                                    ...
                                                    player = roomlist[index].table.getPlayerByName(element.username);
                                                ...
                                                let seatnumber;
                                                if (element.mode == 'bot') {
                                                    if (element.moveroom > 0 && roomlist[index].roomid == element.moveroom) {
                                                        seatnumber = roomlist[index].table.addPlayer({
                                                            playerName: element.username,
                                                            chips: element.balance,
                                                            avatar: element.avatar,
                                                    ...
                                                    if (element.moveroom == 0 || element.moveroom > 0 && roomlist[index].roomid == element.moveroom) {
                                                        seatnumber = roomlist[index].table.addPlayer({
                                                            playerName: element.username,
                                                            chips: element.balance,
                                                            avatar: element.avatar,
                    ...
                    setTimeout(() => {
                        ...
                        getUsername(useridArray, function (data) {
                            ...
                            io.in('r' + roomlist[index].roomid).emit('TABLE_REPOSITION', emitdata);
                            io.in('r' + roomlist[index].roomid).emit('CURRENT_ROOM_NAMES', { result: usernames });
                        });
                        ...
                        setTimeout(() => {
                            ...
                            io.sockets.in('r' + roomlist[index].roomid).emit('TAKE_SEAT_PLAYERS', emitdata);
                        }, 10);
                    }, Reposition_time);
                    let seatedPlayers = roomlist[index].playerlist.filter(x => x.leaveenterflag == 1);
                    let seatnormals = seatedPlayers.filter(x => x.mode == 'normal');
...
async function checkrooms_tournament(roomIndex) {
    ...
    for (let index = 0; index < roomlist.length; index++) {
        if (room != roomlist[index] && roomlist[index].gamemode == 'tournament') {
            ...
            if (emtpy_seats_num >= room.playerlist.length && check_room.playerlist.length >= room.playerlist.length) {
    ...
    setTimeout(() => {
        if (checked == room.playerlist.length) {
            ...
            if (io.nsps['/'].adapter.rooms['r' + room.roomid] != undefined) {
                for (socketID in io.nsps['/'].adapter.rooms['r' + room.roomid].sockets) {
            ...
            for (let i = 0; i < room.playerlist.length; i++) {
                ...
                let filtered = roomSockets.filter(x => x.nickname == room.playerlist[i].username);
                if (filtered.length > 0) {
                    ...
                    getUsername(useridArray, function (data) {
                        ...
                        filtered[0].clientSocket.emit('CURRENT_ROOM_NAMES', { result: usernames });
                    });
                ...
                let pos = 0;
                while (true) {
                    if (to_room.table.players[pos] == undefined || to_room.table.players[pos].playerName == "Empty seat") {
                        let a = positions.filter(x => x == pos);
                        if (a.length == 0) {
                            break;
                        } else
                ...
                setTimeout(() => {
                    ...
                    io.sockets.in('r' + to_room.roomid).emit('TAKE_SEAT_PLAYERS', emitdata);
                }, 50);
            }
            setTimeout(() => {
...
function removetable(roomIndex) {
    let seatedPlayers = roomlist[roomIndex].table.players.filter(x => x.playerName != "Empty seat");
...
exports.Action = function (info) {
    for (let index = 0; index < roomlist.length; index++) {
        if (roomlist[index].roomid == info.room_id) {
            ...
            if (player.table.currentPlayer != info.position && checkIndex(player, info.position)) return;
            exports.removetimeout(index);
...
exports.Check = function (index, playerid) {
    if (roomlist[index].table != undefined) {
        ...
        if (player) {
            ...
            io.in('r' + roomlist[index].roomid).emit('PLAYER_ACTION_RESULT', emitdata);
            player.Check();
        }
    }
};
    ...
    if (roomlist[index].table != undefined) {
        if (roomlist[index].table != undefined) {
            ...
            if (player) {
                ...
                io.sockets.in('r' + roomlist[index].roomid).emit('PLAYER_ACTION_RESULT', emitdata);
                player.fold();
            }
        }
    }
    ...
    for (let index = 0; index < roomlist.length; index++) {
        if (roomlist[index].roomid == roomid) {
            for (let i = 0; i < roomlist[index].playerlist.length; i++) {
                ...
                if (element.username == username) {
                    let alone = false;
                    if (element.leaveenterflag == 0) // sit-down state: sitting in a seat but not playing
                    {
                        let player = roomlist[index].table.getPlayerByName(username);
                        if (player != undefined) {
                            //console.log("8");
                    ...
                    else // The status taken a part in game directly
                    {
                        let sitteds = exports.getPlayersSitted(roomlist[index]);
                        ...
                        setTimeout(() => {
                            ...
                            setTimeout(() => {
                                let seatedPlayers = roomlist[index].table.players.filter(x => x.playerName != "Empty seat" && x.isEmptySeat == false);
                                //if (seatedPlayers.length <= 1) {
                                if (seatedPlayers.length == 0) {
                    ...
                    setTimeout(() => {
                        let onlineplayers = roomlist[index].playerlist.filter(x => x.leaveenterflag != 0);
                        ...
                        io.sockets.in('r' + roomid).emit('PLAYER_VIEW_TABLE_RESULT', emitdata);
                        setTimeout(() => {
                            let takeSeats = [];
                            for (let i = 0; i < roomlist[index].playerlist.length; i++) {
                                const element = roomlist[index].playerlist[i];
                        ...
                        setTimeout(() => {
                            ...
                            setTimeout(() => {
                                ...
                                if (roomlist[index].gamemode == 'tournament')
                                    ...
                                    checknum = 1;
                                if (seatedPlayers.length == 0 && seatedPlayers.filter(x => x.mode == 'bot').length == 0) {
                                    console.log('>>>');
                                    roommanager.BotsCreating(roomlist[index].roomid, roomlist[index].buyin_min, roomlist[index].seatlimit);
                                }
                                if (seatedPlayers.length <= checknum || real_seatedPlayers.length <= checknum) {
                                    exports.removetimeout(index);
                                    element.leaveenterflag = 1;
                                    let seatnumber = roomlist[index].table.addPlayer({
                                        playerName: element.username,
                                ...
                                } else if (seatedPlayers.length > checknum || real_seatedPlayers.length > checknum) {
                                    console.log('wait for bb');
                                    io.sockets.in('r' + info.room_id).emit('BUYIN_WAITFORBB', info);
                                    // let seatBots = roomlist[index].playerlist.filter(x => x.mode == 'bot');
...
function Record_Won_History(winner, prize, wining_hand, wining_cards, handrankVal) {
    let collection = database.collection('User_Data');
    let query = {
        userid: winner
    };
...
exports.Leave = function (socket, data) {
    ...
    console.log('LEAVE roomindex', index, 'username', username, 'position', position);
    let leftPoints = 0;
    let isExist = false;
    let leavePos = 0;
    setTimeout(() => {
        if (roomlist[index]) {
            for (let i = 0; i < roomlist[index].playerlist.length; i++) {
                if (roomlist[index].playerlist[i].username == username) {
                    ...
                    if (roomlist[index].playerlist[i].mode == 'bot') BotManager.leaveBot(username);
        ...
        setTimeout(() => {
            if (isExist == true) {
                ...
                setTimeout(() => {
                    ...
                    io.sockets.in('r' + data.room_id).emit('PLAYER_LEAVE_RESULT', emitdata);
...
exports.SuccessSpin = function (socket, data) {
    let spinBalances = [100000000, 200000000, 300000000, 400000000, 500000000, 600000000, 700000000, 800000000];
    ...
    collection.find().toArray(function (err, docs) {
        if (err) {
            ...
            if (docs.length > 0) {
                ...
                isAdd = true;
                let date = new Date();
                let d1 = new Date(date.getFullYear(), date.getDay() + 1, date.getDate(), date.getHours(), 0, 0);
                maxTime = d1.getTime();
            }
                ...
                if (docs.length > 0) {
                    for (let i = 0; i < docs.length; i++) {
                        ...
                        let item = {
                            ...
                            remain_time: msToTime(element.start_time.getTime() - new Date().getTime()),
...
exports.OnDisconnect = function (socket) {
    ...
    if (socket.room == undefined || username == undefined)
        return;
