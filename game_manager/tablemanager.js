      ...
      try {
        ...
        if (call == 0) minRaise = this.bigBlind;
          ...
          if (current_Bet == 0) {
            ...
            if (call < this.bigBlind) minRaise = call + this.bigBlind;
        ...
        if (canCheck) info.action = "check";
          ...
          canCall &&
          ((this.isRaise && (botgoodcards || goodcards)) || !this.isRaise)
        ) {
          ...
          if (this.bigBlinds.indexOf(this.bigBlind) == -1) {
            if (goodcards == true || player.win == true) {
              ...
              if (num1 > 4) {
                ...
                let randomNumber =
                  raiseRandom[Math.floor(Math.random() * raiseRandom.length)];
                if (canRaise) {
                  ...
                  if (info.bet < player.chips) {
                    ...
                    for (let i = 0; i < this.table.players.length; i++) {
                      ...
                      if (
                        this.table.players[i] != undefined &&
                        this.table.players[i].chips != undefined &&
                    ...
                    if (buff < player.chips) {
                      if (this.table.game.bets.length > 0) {
                        info.bet =
                          buff + this.table.game.bets[index] - player.GetBet();
                      } else {
              ...
              this.collection_UserData.updateOne(
                query,
                {
                  $set: {
                    points:
                      result.points +
                      parseInt(config.JACKPOT_WINS_ROYALFLASH.array[ind]),
                  },
                },
                function (err) {
                  if (err) throw err;
              ...
              this.collection_UserData.updateOne(
                query,
                {
                  $set: {
                    points:
                      result.points +
                      parseInt(config.JACKPOT_WINS_4KINDS.array[ind]),
                  },
                },
                function (err) {
                  if (err) throw err;
      ...
      let interval = setInterval(() => {
        ...
        if (count <= removeCount) {
          let player = this.table.players.find(
            (p) => p && p.mode == "bot" && p.playerName != "Empty seat"
          );
...
TableManager.prototype.standUp_force = function (player, bankrupt) {
  this.standUp(
    { userid: player.playerID, username: player.playerName, mode: player.mode },
    null,
    bankrupt
  );
};
  ...
  try {
    ...
    if (player) {
      ...
      player = this.table.players.find(
        (p) => p && p.playerID == info.userid && p.playerName == info.username
      );
